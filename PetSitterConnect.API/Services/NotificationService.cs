...
public class NotificationService : INotificationService
{
    ...
    public async Task SendReviewReceivedNotificationAsync(Review review)
    {
        ...
        var reviewerName = review.Reviewer?.FullName ?? "Someone";
        var petName = review.Booking?.PetCareRequest?.Pet?.Name ?? "your pet";
        var stars = new string('⭐', review.Rating);
        
        var message = $"{reviewerName} left you a {review.Rating}-star review ({stars}) for caring for {petName}!";
